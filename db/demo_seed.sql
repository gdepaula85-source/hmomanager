-- demo_seed.sql
-- Wipes and re-seeds the demo organisation with realistic HMO + SA portfolio data.
-- Idempotent — safe to run on any schedule. The demo org id is fixed so all references
-- stay stable across resets.
--
-- Seed contents:
--   1 org (LandlordApp Demo, business plan, FREE GRANT)
--   1 limited company
--   3 landlords
--   5 properties (4 HMO, 1 Single Let / STR-enabled; spread across London + Manchester)
--   12 tenants (distributed across the HMOs + the whole-let)
--   5 maintenance jobs (mix of open / in-progress / resolved)
--   ~24 payments (last month paid + current month scheduled)
--   5 recurring expenses
--
-- DOES NOT create the auth user — do that once in Supabase Dashboard → Authentication
-- (see the setup notes at the bottom of this file). Re-running the script preserves the
-- user link in org_members because we key off a fixed org_id.
--
-- Run on piufcteaqmxemidfdoim.

do $$
declare
  demo_org uuid := '00000000-0000-0000-0000-00000000d3d0';  -- fixed demo org id
...
begin
  ...
  on conflict (id) do update set
    ...
    billing_override = excluded.billing_override,
    billing_override_note = excluded.billing_override_note,
    stripe_subscription_id = excluded.stripe_subscription_id;

  -- ── 3. Company (operating entity) ─────────────────────────────────
  ...
  insert into public.properties (id, org_id, name, address, postcode, area, type, rooms, occupied, rent, landlord_rent, landlord_id, landlord_name, maps_url, notes, company_id, room_list, ownership_type, letting_type, bedrooms, status, is_str_enabled) values
    (p1, demo_org, '27 Vassall Road',    '27 Vassall Road, London SW9 6TA',    'SW9 6TA',  'Oval',            'HMO',        4, 4, 3250, 1900, ll1, 'James Whitfield', 'https://maps.google.com/?q=27%20Vassall%20Road%20London%20SW9%206TA', 'Four-room converted HMO, Article 4 area.', co1,
    ...
    (p2, demo_org, '4 Grange Park Road', '4 Grange Park Road, London CR7 8QA', 'CR7 8QA',  'Thornton Heath',  'HMO',        1, 1, 900,  650,  ll2, 'Priya Shah', 'https://maps.google.com/?q=4%20Grange%20Park%20Road%20London%20CR7%208QA', 'Single en-suite studio unit.', co1,
      '[{"n":1,"type":"Studio","price":900,"status":"occupied"}]'::jsonb,
      'managed', 'hmo', null, 'active', false),

    (p3, demo_org, '22 Elm Grove',       '22 Elm Grove, London SE15 5DB',      'SE15 5DB', 'Peckham',         'HMO',        3, 3, 2400, 1400, ll1, 'James Whitfield', 'https://maps.google.com/?q=22%20Elm%20Grove%20London%20SE15%205DB', 'Three-room HMO above shop.', co1,
    ...
    (p4, demo_org, '31 Vassall Road',    '31 Vassall Road, London SW9 6TA',    'SW9 6TA',  'Oval',            'Single Let', 1, 1, 1800, 0,    null, '',               'https://maps.google.com/?q=31%20Vassall%20Road%20London%20SW9%206TA', 'Owned whole-let. Hybrid AST + occasional Airbnb.', co1,
      '[{"n":1,"type":"Whole Property","price":1800,"status":"occupied","isWholeProperty":true}]'::jsonb,
      'owned', 'whole', 2, 'active', true),

    (p5, demo_org, '9 Argyll Street',    '9 Argyll Street, Manchester M15 1EA','M15 1EA',  'Hulme',           'HMO',        4, 3, 2325, 1800, ll3, 'Marcus Okonkwo', 'https://maps.google.com/?q=9%20Argyll%20Street%20Manchester%20M15%201EA', 'Manchester HMO, one SA-enabled room for flex.', co1,
  ...
  -- ── 8. Payments ───────────────────────────────────────────────────
  -- Last month: every tenant paid. Dates anchor to "first of this month minus
  -- 15/14 days" so they always fall in the previous calendar month, regardless
  -- of when in the month the demo is reset. (Old code used `_today - 14` which
  -- leaked into the current month if today was past the 15th — so the
  -- dashboard's monthly Collected total picked them up while the rent page's
  -- weekly view did not, making the two screens disagree.)
  ...
  select gen_random_uuid(), demo_org, t.id, t.name, t.property_name, t.rent, t.method, 'paid',
         (date_trunc('month', _today) - interval '15 days')::date,
         (date_trunc('month', _today) - interval '14 days')::date,
         'rent'
  from public.tenants t where t.org_id = demo_org;

  -- Current month: all scheduled, except one overdue to make arrears realistic.
  ...
  -- ── 10. SA / Airbnb income on the STR-enabled property ────────────
...
-- Force PostgREST to pick up any schema changes from a prior migration.
