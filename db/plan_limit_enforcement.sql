...
begin
  ...
  if tg_table_name = 'properties' then
    ...
    if v_count >= v_cap_properties then
      raise exception 'Plan limit reached: max % properties for plan %.', v_cap_properties, coalesce(v_plan, 'free')
        using errcode = 'P0001';
    end if;
    ...
    if coalesce(new.status, 'active') <> 'inactive' then
      ...
      if v_count >= v_cap_tenants then
        raise exception 'Plan limit reached: max % active tenants for plan %.', v_cap_tenants, coalesce(v_plan, 'free')
          using errcode = 'P0001';
      end if;
    end if;
    ...
    if v_count >= v_cap_users then
      raise exception 'Plan limit reached: max % users for plan %.', v_cap_users, coalesce(v_plan, 'free')
        using errcode = 'P0001';
    end if;
  end if;
