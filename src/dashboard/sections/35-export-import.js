...
function _dmPlanCaps(plan, org) {
  ...
  var UNLIMITED_CAP = 2147483647;
  // Admin-granted free access (FREE GRANT / partner orgs) → unlimited on everything.
  // Matches superadmin intent: billing_override='free' waives billing AND plan caps.
  if (o && String(o.billing_override || '').toLowerCase() === 'free') {
    return { properties: UNLIMITED_CAP, tenants: UNLIMITED_CAP };
  }
  // Time-boxed free grant: if free_until date is today or later, treat as unlimited.
  if (o && o.free_until) {
    var fuTs = new Date(o.free_until).getTime();
    var todayTs = new Date().setHours(0,0,0,0);
    if (!isNaN(fuTs) && fuTs >= todayTs) {
      return { properties: UNLIMITED_CAP, tenants: UNLIMITED_CAP };
    }
  }
  ...
  // Use admin-configured plan limits from saas_config if loaded; fall back to hardcoded defaults.
  var planCfg = cfg.planConfig && cfg.planConfig[p];
  var UNLIMITED = 2147483647;
  var propLimit = planCfg
    ? (planCfg.properties === 0 ? UNLIMITED : (planCfg.properties || UNLIMITED))
    : (p === 'enterprise' ? UNLIMITED : p === 'business' ? 60 : p === 'professional' ? 25 : p === 'starter' ? 15 : p === 'trial' ? 5 : 3);
  var tenantLimit = planCfg
    ? (planCfg.tenants === 0 ? UNLIMITED : (planCfg.tenants || UNLIMITED))
    : (p === 'enterprise' || p === 'business' || p === 'professional' ? UNLIMITED : p === 'starter' ? 75 : p === 'trial' ? 30 : 15);
  return { properties: propLimit, tenants: tenantLimit };
}
...
function _dmTemplate() {
  ...
  if(entity==='properties') {
    ...
    sample=['99 Example Street','99 Example Street London SW2 1AA','SW2 1AA','Brixton',5,2500,3200,4,'John Smith','447911000001','Managed','https://maps.google.com','Good condition','My Property Company Ltd'];
  ...
  reader.onload = function(e) {
    try {
      ...
      if(!data.tenants || !data.properties) {
        alert('\u26A0\uFE0F Invalid backup file. Please use a landlordapp.io export file.');
        return;
      }
      ...
      SAVE_KEYS_LOCAL.forEach(function(k){
        if(data[k] !== undefined) {
          state[k] = data[k];
          localStorage.setItem(orgStorageKey('pm_local_' + k), JSON.stringify(data[k]));
        }
      });
      rebuildAllSchedules();
      render();
