...
function runLocalPortfolioAnalysis() {
  ...
  var occupancyPct=totalRooms?Math.min(100,Math.max(0,Math.round(occupiedRooms/totalRooms*100))):0;
  ...
  var noticeTenants=tenants.filter(function(t){return t.status==='notice_given';});
  ...
  // Render — uses explicit light colors for dark .ai-card background
  var _t='#E2E8F0', _m='rgba(226,232,240,.5)', _d='rgba(226,232,240,.35)', _bg='rgba(255,255,255,.06)', _bd='rgba(255,255,255,.08)';
  var html='<div style="padding:14px 18px">';

  // Score bar
  html+='<div style="display:flex;align-items:center;gap:14px;margin-bottom:16px;padding-bottom:14px;border-bottom:1px solid '+_bd+'">'
    ...
    +'<div style="font-size:11px;color:'+_m+'">'+props.length+' properties · '+occupancyPct+'% occupied · '+fmt(totalNet)+'/mo net · '+openMaint.length+' open jobs</div>'
    +'<div style="margin-top:6px;height:6px;border-radius:3px;background:'+_bd+';overflow:hidden"><div style="height:100%;width:'+score+'%;background:'+scoreColor+';border-radius:3px;transition:width .5s"></div></div>'
    +'</div>'
    +'<button onclick="runLocalPortfolioAnalysis()" style="padding:6px 12px;border-radius:8px;border:1px solid '+_bd+';background:'+_bg+';font-size:11px;color:'+_m+';cursor:pointer;font-family:inherit;flex-shrink:0">&#x21BB; Refresh</button>'
    +'</div>';
  ...
  if(alerts.length>0){
    ...
    alerts.forEach(function(a){
      html+='<div style="display:flex;align-items:center;gap:8px;padding:8px 12px;background:'+_bg+';border:1px solid '+_bd+';border-left:3px solid '+a.c+';border-radius:8px">'
        +'<span style="color:'+a.c+';flex-shrink:0">'+a.icon+'</span>'
        +'<span style="font-size:12px;color:'+_t+'">'+a.t+'</span></div>';
    });
    html+='</div>';
  }
  ...
  if(insights.length>0){
    html+='<div style="font-size:10px;font-weight:700;color:'+_m+';text-transform:uppercase;letter-spacing:.06em;margin-bottom:8px">Analysis</div>';
    html+='<div style="display:flex;flex-direction:column;gap:5px;margin-bottom:14px">';
    insights.forEach(function(ins){
      html+='<div style="display:flex;align-items:flex-start;gap:7px;font-size:12px;color:'+_t+';line-height:1.5">'
  ...
  // Actions
  if(actions.length>0){
    html+='<div style="font-size:10px;font-weight:700;color:'+_m+';text-transform:uppercase;letter-spacing:.06em;margin-bottom:8px">Recommended Actions</div>';
  ...
  html+='<div style="margin-top:10px;font-size:10px;color:'+_d+'">Local analysis — runs offline, no API required</div>';
  html+='</div>';
...
function renderDashboardAgent(){
  var el=document.getElementById('ai-agent-output');
  if(!el) return;
  var cached=localStorage.getItem(orgStorageKey('pm_agent_cache'));
  var cacheTs=+(localStorage.getItem(orgStorageKey('pm_agent_cache_ts'))||0);
