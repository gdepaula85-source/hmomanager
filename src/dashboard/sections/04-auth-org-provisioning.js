...
function isPaidPlanForCheckout(plan) {
  ...
  // Paid self-serve tiers. Enterprise is self-serve too — no "contact sales" wall.
  // Custom plans added via superadmin are assumed paid (checkout may need Stripe price ID config).
  if (p === 'trial' || p === 'free' || p === '') return false;
  return true;
}
...
/** Paid plan without checkout: real Stripe sub (sub_…) or superadmin-granted marker (see superadmin.js). */
function hasBillingExemptionOrSubscription(org) {
  if (!org) return false;
  // Admin-granted free access (FREE GRANT / partner orgs / demo) bypasses the checkout gate.
  // Matches the same flag used by _dmPlanCaps to waive plan caps.
  if (String(org.billing_override || '').toLowerCase() === 'free') return true;
  // Time-boxed free grant: bypass while free_until is today or later.
  if (org.free_until) {
    var fu = new Date(String(org.free_until).split('T')[0] + 'T00:00:00');
    var today = new Date(); today.setHours(0,0,0,0);
    if (!isNaN(fu.getTime()) && fu >= today) return true;
  }
  var sid = String(org.stripe_subscription_id || '').trim();
  if (!sid) return false;
  if (sid === 'manual') return true;
  if (sid.indexOf('sub_') === 0) return true;
  return false;
}

function needsPaidCheckoutGate(org) {
  return !!(org && isPaidPlanForCheckout(org.plan) && !hasBillingExemptionOrSubscription(org));
}
...
async function syncUsersFromOrgMembers(session){
  ...
  try{
    ...
    // Pull invited_name / invited_email too — these are denormalized at invite
    // time (see /api/users/invite + db/org_members_invited_name.sql) so OTHER
    // admins see real names like "Fred Jones" instead of "User f0b212".
    // Wrapped in a try/fallback so projects that haven't run the migration
    // still work — they just keep the placeholder behaviour.
    var q = await supa.from('org_members')
      .select('user_id, role, created_at, invited_name, invited_email')
      .eq('org_id', _currentOrgId)
      .order('created_at', {ascending:true});
    if (q.error) {
      var errCode = String(q.error.code || '');
      var errMsg  = String(q.error.message || '');
      if (errCode === '42703' || errCode === 'PGRST204' || /invited_name|invited_email/i.test(errMsg)) {
        // Migration not yet applied — re-fetch with the original columns only.
        q = await supa.from('org_members')
          .select('user_id, role, created_at')
          .eq('org_id', _currentOrgId)
          .order('created_at', {ascending:true});
      }
      if (q.error) { console.warn('syncUsersFromOrgMembers:', q.error); return; }
    }
    ...
    var mapped = (q.data||[]).map(function(m){
      ...
      //   1. Locally-known name from prior session (preserves edits made on this device)
      //   2. Server-stamped invited_name (visible to all admins, no localStorage dep)
      //   3. Current session name (only useful when isMe)
      //   4. Last-resort placeholder using the user_id tail
      var serverName = (m.invited_name && String(m.invited_name).trim()) || '';
      var name = prev.name
        || serverName
        || (isMe ? currentName : ('User ' + uid.slice(-6)));
...
async function resolveOrg(session) {
  ...
  // Pull ALL memberships — needed so an `invited_to_org` metadata flag can
  // override a stale trial-org membership left behind by an earlier broken invite.
  var { data: memberships, error: memberErr } = await supa.from('org_members')
  ...
  var allMemberships = memberships || [];

  // If the auth user was invited to a specific org (server-side endpoint stamps
  // `invited_to_org` in user_metadata before sending the email), prefer THAT
  // membership over any earlier one. Without this, a user who had previously
  // signed up via the old broken `signInWithOtp` invite flow would get pinned
  // to the auto-provisioned trial org forever.
  var invitedOrgIdMeta = session.user.user_metadata && session.user.user_metadata.invited_to_org;
  var membership = null;
  if (invitedOrgIdMeta) {
    // Invited staff: ONLY use the matching membership. Step 2 below recovers
    // via upsert if the row is missing. We never fall back to allMemberships[0]
    // for an invited user — that path landed staff in a stale trial org and
    // triggered the brand-new-user welcome_signup flow.
    membership = allMemberships.find(function(m){ return String(m.org_id) === String(invitedOrgIdMeta); }) || null;
  } else {
    membership = allMemberships.length > 0 ? allMemberships[0] : null;
  }
  ...
  // 2. No org found via membership lookup — but if this user was invited as
  // internal staff, the server attached them to an existing org. Re-check by
  // user_metadata.invited_to_org and short-circuit the new-org provisioning.
  var invitedOrgId = session.user.user_metadata && session.user.user_metadata.invited_to_org;
  var invitedRole  = (session.user.user_metadata && session.user.user_metadata.invited_role) || 'viewer';
  if (invitedOrgId) {
    // Ensure the org_members row exists (server pre-creates it, but the trigger
    // might have raced or the row may have been deleted). Idempotent upsert.
    try {
    ...
    } catch(_e) { /* RLS will reject if user isn't supposed to be here */ }
    var { data: invOrg } = await supa.from('organisations').select('*').eq('id', invitedOrgId).maybeSingle();
    if (invOrg) {
      _currentOrgId = invOrg.id;
      _currentMemberRole = invitedRole;
      state._currentOrg = invOrg;
      console.log('Joined invited org:', invOrg.name, invOrg.id);
      return true;
    }
    // CRITICAL: a user with `invited_to_org` set is a staff invitee. They must
    // NEVER be auto-provisioned a new trial org — that's exactly how staff
    // members were ending up with trial-user welcome emails and their own
    // empty workspace. If the invited org couldn't be resolved (deleted,
    // RLS-blocked, or the server-side org_members upsert silently failed),
    // surface an error so the admin can re-invite them.
    showInviteOrgUnavailable();
    return false;
  }

  // 3. Truly brand-new user — provision their own org.
...
function showInviteOrgUnavailable() {
  document.body.innerHTML = `
    <div style="min-height:100vh;background:#0B0D12;display:flex;align-items:center;justify-content:center;font-family:system-ui,sans-serif;padding:20px">
      <div style="background:#13161E;border:1px solid rgba(255,77,106,.3);border-radius:16px;padding:40px;max-width:460px;width:100%;text-align:center">
        <div style="font-size:48px;margin-bottom:16px">✉️</div>
        <div style="font-size:22px;font-weight:700;color:#fff;margin-bottom:8px">Invite couldn't be completed</div>
        <div style="font-size:14px;color:#7A8099;margin-bottom:24px;line-height:1.6">
          We couldn't attach you to the workspace you were invited to. The link may have expired, or the workspace may no longer exist.<br><br>
          Please ask the admin who invited you to send a fresh invite.
        </div>
        <button onclick="doLogOut()" style="display:inline-block;padding:13px 28px;background:#00D897;color:#000;font-weight:700;font-size:15px;border-radius:10px;border:none;cursor:pointer;font-family:inherit">Sign out</button>
      </div>
    </div>`;
}
  ...
  document.body.innerHTML = `
    <div style="min-height:100vh;background:#0B0D12;display:flex;align-items:center;justify-content:center;font-family:system-ui,sans-serif;padding:20px">
      <div style="background:#13161E;border:1px solid rgba(255,255,255,.1);border-radius:16px;padding:40px;max-width:460px;width:100%;text-align:center">
        ...
        <a href="mailto:support@landlordapp.io?subject=landlordapp.io Account&body=Hi, my account (${orgName}) is ${status}. Please help." style="display:inline-block;padding:13px 28px;background:#00D897;color:#000;font-weight:700;font-size:15px;border-radius:10px;text-decoration:none;margin-bottom:12px">Contact Support</a>
        <br>
        <button onclick="doLogOut()" style="background:none;border:none;color:#7A8099;font-size:13px;cursor:pointer;margin-top:8px;font-family:inherit">Sign out</button>
      </div>
    </div>`;
...
supa.auth.onAuthStateChange(function(event, session) {
  ...
  resolveOrg(session).then(function(ok) {
    ...
    loadState().then(function(loaded){
      ...
      if (!canSee(state.page)) state.page = (typeof fallbackVisiblePage === 'function') ? fallbackVisiblePage() : 'dashboard';
      render();
