...
async function emailDocToTenants(propId, docId) {
  var prop = state.properties.find(function(p){ return p.id === propId; });
  if (!prop) { showToast('Property not found', 'error'); return; }
  var doc = (state.propDocs && state.propDocs[propId] || []).find(function(d){ return d.id === docId; });
  if (!doc) { showToast('Document not found', 'error'); return; }
  var tenants = state.tenants.filter(function(t){ return t.property === prop.name && t.status === 'active' && t.email; });
  if (tenants.length === 0) { showToast('No active tenants with email at this property', 'error'); return; }
  if (!confirm('Email "' + doc.type + '" to ' + tenants.length + ' tenant(s) at ' + prop.name + '?')) return;
  var cx = getCompanyEmailContext();
  var sent = 0;
  for (var i = 0; i < tenants.length; i++) {
    var t = tenants[i];
    var first = (t.name || 'there').trim().split(/\s+/)[0];
    var html = buildPropertyDocShareHtml({
      firstName: first, docType: doc.type || doc.name, property: prop.name,
      companyName: cx.companyName, downloadUrl: doc.dataUrl || ''
    });
    var ok = await sendEmail(t.email, doc.type + ' - ' + prop.name, 'Property document shared', 'tenant', {
      html: html, templateId: 'property_doc_share', tenant: { firstName: first }
    });
    if (ok) sent++;
  }
  showToast('Sent document to ' + sent + ' tenant(s)', 'success');
}
...
function removePropDoc(propId,docId){
  ...
  state.propDocs[propId]=state.propDocs[propId].filter(function(d){return d.id!==docId;});
  supaDelete('property_docs', docId);
