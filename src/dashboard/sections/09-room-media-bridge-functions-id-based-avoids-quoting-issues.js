...
function toggleRoomAvailByEid(el) {
  ...
  r._hidden = !r._hidden;
  saveState();
  // Defensive: ensure we stay on the Rooms page after save + render. Some hot
  // paths have routed us to Settings after a property upsert error — pin it.
  state.page = 'rooms';
  render();
  showToast(r._hidden?'Room marked Unavailable — moved to bottom':'Room marked Available', r._hidden?'error':'success');
}
...
function render() {
  renderNav();
  const pages = {dashboard:renderDashboard,properties:renderProperties,tenants:renderTenants,communication:renderCommunication,rent:renderRent,expenses:renderExpenses,maintenance:renderMaintenance,landlords:renderLandlords,rooms:renderRooms,diary:renderDiary,reports:renderReports,settings:renderSettings,users:renderUsers,import:renderImport};
  try {
  ...
  // Post-render hooks — anything that needs to wire DOM events (canvas, etc.)
  // since <script> tags inside innerHTML never execute.
  if (state.page === 'settings' && typeof _initCfgSigCanvas === 'function') {
    try { _initCfgSigCanvas(); } catch(e3) { console.warn('Settings sig canvas init failed:', e3); }
  }
  // Onboarding wizard overlay — idempotent; no-op if already completed/dismissed.
  if (typeof showOnboardingWizardIfNeeded === 'function') {
    try { showOnboardingWizardIfNeeded(); } catch(e2) { console.warn('Onboarding hook failed:', e2); }
  }
  // Sticky activation checklist — appears post-wizard for orgs still missing
  // milestones (property, tenant, first rent, first compliance doc).
  if (typeof showOnboardingStickyChecklistIfNeeded === 'function') {
    try { showOnboardingStickyChecklistIfNeeded(); } catch(e2) { console.warn('Sticky checklist hook failed:', e2); }
  }
}
