...
// toggleRoomAvailByEid: canonical impl lives in
// sections/09-room-media-bridge-functions-id-based-avoids-quoting-issues.js.
// The orphan copy here was overridden at bundle time by the later section
// definition and caused confusion when we edited the wrong copy — removed.
...
function renderDashboard() {
  ...
  var collectedRentAmt = ms.pays.filter(function(p){return p.status==='paid' && (p.incomeSource||'rent')==='rent';}).reduce(function(a,p){return a+p.amount;},0);
  var rentPaymentCount = ms.pays.filter(function(p){return p.status==='paid' && (p.incomeSource||'rent')==='rent';}).length;
  // STR income for the same selected month (from all payments, scoped to month)
  var _msMonthStart = new Date(selMonth+'-01');
  var _msMonthEnd   = new Date(_msMonthStart.getFullYear(), _msMonthStart.getMonth()+1, 0, 23,59,59);
  var strThisMonth = (state.payments||[]).filter(function(p){
    if(p.incomeSource !== 'airbnb') return false;
    var d = p._paidDateRaw ? new Date(p._paidDateRaw) : (p.periodEnd ? new Date(p.periodEnd) : null);
    return d && d >= _msMonthStart && d <= _msMonthEnd;
  });
  var strThisMonthAmt = strThisMonth.reduce(function(s,p){return s+(+p.amount||0);},0);
  var strEnabledCount = (state.properties||[]).filter(function(p){return p.isStrEnabled && p.status!=='archived';}).length;
  ...
  html += '</div>';
  // Row 2: Actual collected + STR income (only when any STR property exists) + costs
  if(strEnabledCount > 0) {
    html += '<div class="kpi-grid" style="grid-template-columns:repeat(5,1fr);margin-bottom:22px">';
    html += kpi('Rent collected', fmt(collectedRentAmt), rentPaymentCount+' rent payments', '#10B981', '&#x2705;');
    html += kpi('STR income', fmt(strThisMonthAmt), strThisMonth.length+' '+(strThisMonth.length===1?'entry':'entries')+' &middot; '+strEnabledCount+' STR '+(strEnabledCount===1?'property':'properties'), '#FF5A5F', '&#x1F6CF;&#xFE0F;');
    ...
    html += '</div>';
    // Stale-period nudge — for any STR-enabled property without an entry this month
    var staleStrProps = (state.properties||[]).filter(function(prop){
      if(!prop.isStrEnabled || prop.status==='archived') return false;
      var hasThisMonth = (state.payments||[]).some(function(pay){
        if(pay.incomeSource !== 'airbnb') return false;
        if(pay.property !== prop.name && pay.propertyName !== prop.name) return false;
        var d = pay._paidDateRaw ? new Date(pay._paidDateRaw) : (pay.periodEnd ? new Date(pay.periodEnd) : null);
        return d && d >= _msMonthStart && d <= _msMonthEnd;
      });
      return !hasThisMonth;
    });
    if(staleStrProps.length) {
      var nowMonthLbl = _msMonthStart.toLocaleDateString('en-GB',{month:'long',year:'numeric'});
      html += '<div style="background:linear-gradient(135deg,#FFF1F2,#fff);border:1px solid #FECDD3;border-radius:10px;padding:11px 14px;margin-bottom:18px;display:flex;justify-content:space-between;align-items:center;gap:12px;flex-wrap:wrap">';
      html += '<div style="font-size:13px;color:#E04E53"><strong>🛏️ '+staleStrProps.length+' Airbnb '+(staleStrProps.length===1?'property':'properties')+' missing income for '+nowMonthLbl+':</strong> '+staleStrProps.slice(0,3).map(function(p){return esc(p.name);}).join(', ')+(staleStrProps.length>3?' (+'+(staleStrProps.length-3)+' more)':'')+'</div>';
      html += '<button data-pid="'+staleStrProps[0].id+'" onclick="openStrIncomeModal(this.dataset.pid)" style="padding:7px 14px;border-radius:8px;border:none;background:#FF5A5F;color:#fff;font-size:12px;font-weight:700;cursor:pointer;font-family:inherit;white-space:nowrap">Log STR income</button>';
      html += '</div>';
    }
  } else {
    // Original 4-tile layout when no STR properties exist (no regression)
    html += '<div class="kpi-grid kpi-4" style="margin-bottom:22px">';
    html += kpi('Collected', fmt(collectedRentAmt), rentPaymentCount+' payments', '#10B981', '&#x2705;');
    html += kpi('Landlord Costs', fmt(ms.landlord), pct(ms.landlord,ms.expectedIncome||1)+'% of income', '#E8375A', '&#x1F3E6;');
    html += kpi('Operating Costs', fmt(ms.opex), 'Staff + property + overhead', '#F59E0B', '&#x2699;&#xFE0F;');
    html += kpi('Active Tenants', state.tenants.filter(function(t){return t.status==='active';}).length, state.tenants.filter(function(t){return t.status==='notice_given';}).length+' on notice', '#3B82F6', '&#x1F465;');
    html += '</div>';
...
function renderProperties() {
  ...
  return `
    ...
    <div class="filters">
      ...
      ${(function(){
        var complianceIssues = state.properties.filter(function(p){
          if(p.status==='archived') return false;
          if(typeof getPropertyComplianceStatus !== 'function') return false;
          var cs = getPropertyComplianceStatus(p);
          return cs.level === 'critical' || cs.level === 'warning' || cs.level === 'missing';
        }).length;
        return ['all','owned','managed','str','compliance','profitable','loss','vacant','archived'].map(function(v){
          var label = v==='loss'?'Loss-Making':v==='vacant'?'Has Vacancies':v==='owned'?'🏠 Owned':v==='managed'?'🤝 Managed':v==='str'?'🛏️ Airbnb / SA':v==='compliance'?('⚠️ Compliance'+(complianceIssues?' ('+complianceIssues+')':'')):v==='archived'?'📦 Archived':v[0].toUpperCase()+v.slice(1);
          return '<button class="filter-btn '+(f===v?'active':'')+'" onclick="state.filters.props=\''+v+'\';render()">'+label+'</button>';
        }).join('');
      })()}
    </div>
    <!-- Portfolio KPI strip -->
    ...
    <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-bottom:16px">
      <div style="background:var(--green-light);border:1px solid #A7F3D0;border-radius:12px;padding:12px;text-align:center">
        <div style="font-size:14px;font-weight:800;color:var(--green);font-family:monospace">${fmt(data.reduce((s,p)=>s + (p.rent||0) + ((typeof getPropStrMonthlyIncome==='function')?getPropStrMonthlyIncome(p):0),0))}</div>
    ...
    <div class="prop-grid">
      ${data.map(p=>{
        ...
        // SA-active properties count as "fully occupied" for the card's occupancy bar
        // — otherwise a pure-SA property with no tenants shows 0% forever even while
        // Airbnb income is flowing in.
        ...
        return `<div class="prop-card${n<0?' loss':''}" onclick="openPropDetail('${p.id}')" style="cursor:pointer">
          ...
          ${(function(){
            if(typeof getPropertyComplianceStatus !== 'function') return '';
            var cs = getPropertyComplianceStatus(p);
            if(cs.level === 'ok') return '';
            var bg = cs.level==='critical'?'#FEF2F2':cs.level==='warning'?'#FFFBEB':'#F8FAFC';
            var bd = cs.level==='critical'?'#FECDD3':cs.level==='warning'?'#FDE68A':'#E2E8F0';
            var col= cs.level==='critical'?'var(--red)':cs.level==='warning'?'var(--amber)':'var(--muted)';
            var icon=cs.level==='critical'?'⚠️':cs.level==='warning'?'⏰':'📋';
            var headline = cs.level==='critical'?'Action needed':cs.level==='warning'?'Expiring soon':'Docs missing';
            // Surface up to 3 most urgent items.
            var top = (cs.items||[]).filter(function(i){return i.state!=='ok'&&i.state!=='no-date';}).slice(0,3);
            return '<div style="margin-top:10px;background:'+bg+';border:1px solid '+bd+';border-radius:10px;padding:9px 11px" onclick="event.stopPropagation();state.propDetailTab=\'docs\';openPropDetail(\''+p.id+'\')">'
              + '<div style="display:flex;justify-content:space-between;align-items:center;gap:6px;margin-bottom:'+(top.length?'6px':'0')+'"><span style="font-size:11px;font-weight:700;color:'+col+'">'+icon+' '+headline+'</span><span style="font-size:10px;color:'+col+';font-weight:700">'+cs.count+' item'+(cs.count===1?'':'s')+'</span></div>'
              + (top.length ? '<div style="display:flex;flex-direction:column;gap:3px">'+top.map(function(i){
                  var short = i.label.replace('Electrical Certificate (EICR)','EICR').replace('Gas Safety Certificate','Gas Safety');
                  var tag = i.state==='expired' ? 'Expired '+Math.abs(i.days)+'d ago'
                          : i.state==='critical' ? (i.days===0?'Expires today':i.days+'d left')
                          : i.state==='warning' ? i.days+'d left'
                          : i.state==='missing' ? 'Not uploaded'
                          : 'No expiry date';
                  var tCol = i.state==='expired'||i.state==='critical'?'var(--red)':i.state==='warning'?'var(--amber)':'var(--muted)';
                  return '<div style="display:flex;justify-content:space-between;font-size:10px;color:var(--text)"><span>'+short+'</span><span style="color:'+tCol+';font-weight:700">'+tag+'</span></div>';
                }).join('')+'</div>' : '')
              +'</div>';
          })()}
...
async function openPropDetail(id) {
  ...
  const detailsTab = `
    ...
    <div style="background:var(--bg);border:1px solid var(--border);border-radius:10px;padding:14px;margin-bottom:16px">
      ...
      <!-- STR (Airbnb) toggle — minimal Phase 1 -->
      <label style="display:flex;align-items:center;gap:10px;padding:10px 11px;margin-top:10px;border-radius:9px;border:2px solid ${p.isStrEnabled?'#FF5A5F':'var(--border)'};background:${p.isStrEnabled?'rgba(255,90,95,.06)':'var(--bg)'};cursor:pointer;transition:all .15s">
        ...
        <div style="flex:1">
          <div style="font-size:12px;font-weight:700;color:${p.isStrEnabled?'#E04E53':'var(--text)'}">🛏️ Generates Airbnb / Short-term rental income</div>
  ...
  // Per-property maintenance count for tab badge
  var _propMaintAll = (state.maintenance||[]).filter(function(m){return m.property === p.name;});
  var _propMaintOpen = _propMaintAll.filter(function(m){return m.status !== 'resolved';}).length;
  const tabs = [
    {v:'details',  l:'Details'},
    {v:'rooms',    l:`Rooms (${p.rooms})`},
    {v:'tenants',  l:`Tenants (${propTenants.length})`},
    {v:'finance',  l:'💰 Finance'},
    {v:'maint',    l:`🔧 Maintenance${_propMaintOpen?' ('+_propMaintOpen+')':''}`},
    {v:'docs',     l:'📁 Docs'},
  ];
  ...
  const maintTab    = (tab==='maint')   ? renderPropMaintenanceTab(p, _propMaintAll) : '<div></div>';
  ...
  document.getElementById('modal-container').innerHTML = `
    <div class="modal-overlay" onclick="if(event.target===this){state.propDetailTab=null;closeModal()}">
      <div class="modal" style="max-width:754px;width:min(754px,calc(100vw - 16px))">
...
function renderPropMaintenanceTab(p, jobs) {
  ...
  // Spend by category for mini bar
  var spendByCat = {};
  ...
  if(spendCats.length) {
    ...
    spendCats.slice(0,6).forEach(function(c){
      var pct = Math.round(spendByCat[c]/maxSpend*100);
      html += '<div style="margin-bottom:8px">'
        ...
        +'<div class="bar-track" style="background:#F1F5F9;height:6px;border-radius:3px;overflow:hidden"><div style="width:'+pct+'%;height:100%;background:var(--blue);border-radius:3px"></div></div>'
        +'</div>';
    });
    html += '</div>';
  }
...
// renderPropFinanceTab: canonical impl lives in sections/20-room-photo-sync-runs-in-background-does-not-block-startup.js.
// The orphan copy here was overridden at bundle time by the later section
// definition and caused confusion when we edited the wrong copy — removed.
...
// savePropDetail: canonical impl lives in sections/20-room-photo-sync-runs-in-background-does-not-block-startup.js.
// The orphan copy here was overridden at bundle time by the later section
// definition and caused confusion when we edited the wrong copy — removed.
...
// Applies org branding to all global UI chrome (sidebar, tab title, favicon).
// Safe to call any time; idempotent. Called from saveBranding and after loadState.
function applyBranding() {
  var cfg = (state && state.config) || {};
  // Browser tab title
  if(cfg.siteTitle) document.title = cfg.siteTitle;
  // Sidebar name (big): prefer siteTitle, else portfolioName, else the
  // branded landlordapp.io wordmark (dot styled in accent colour via HTML).
  ...
  // Favicon (optional — if logo is a data URL or URL)
  if(cfg.logoUrl) {
...
if (typeof window !== 'undefined') { window.badge=badge;window.kpi=kpi;window.btn=btn;window.waLink=waLink;window.waBtn=waBtn;window.getStats=getStats;window.can=can;window.canSee=canSee;window.switchUser=switchUser;window.getPortalUsername=getPortalUsername;window.getPortalPassword=getPortalPassword;window.resetPortalPassword=resetPortalPassword;window.ensurePortalCredentials=ensurePortalCredentials;window.setAppBootMessage=setAppBootMessage;window.hideAppBootOverlay=hideAppBootOverlay;window.normalizeRole=normalizeRole;window.upsertSessionUser=upsertSessionUser;window.showTrialExpired=showTrialExpired;window.showAccountInactive=showAccountInactive;window.rowToLandlord=rowToLandlord;window.rowToProp=rowToProp;window.rowToTenant=rowToTenant;window.rowToPayment=rowToPayment;window.rowToExpense=rowToExpense;window.rowToMaintenance=rowToMaintenance;window.rowToLandlordPayment=rowToLandlordPayment;window.rowToContractor=rowToContractor;window.contractorToRow=contractorToRow;window.landlordToRow=landlordToRow;window.propToRow=propToRow;window.tenantToRow=tenantToRow;window.paymentToRow=paymentToRow;window.expenseToRow=expenseToRow;window.maintenanceToRow=maintenanceToRow;window.landlordPaymentToRow=landlordPaymentToRow;window.renderNav=renderNav;window.quickSetDueDay=quickSetDueDay;window._parseRoomEid=_parseRoomEid;window.handleRoomPhotoChange=handleRoomPhotoChange;window.handleRoomVideoChange=handleRoomVideoChange;window.toggleRoomAvailByEid=toggleRoomAvailByEid;window.saveRoomNotesByEid=saveRoomNotesByEid;window.shareRoomWAByEid=shareRoomWAByEid;window.removeRoomPhotoByEid=removeRoomPhotoByEid;window.removeRoomVideoByEid=removeRoomVideoByEid;window.removeRoomPhotoBtn=removeRoomPhotoBtn;window.removeRoomVideoBtn=removeRoomVideoBtn;window.handlePhotoUploadBtn=handlePhotoUploadBtn;window.handleVideoUploadBtn=handleVideoUploadBtn;window.shareRoomWABtn=shareRoomWABtn;window.saveRoomNotesBtn=saveRoomNotesBtn;window.goto=goto;window.render=render;window.getMonthStats=getMonthStats;window.showChartTip=showChartTip;window.hideChartTip=hideChartTip;window.renderDashboard=renderDashboard;window.renderProperties=renderProperties;window.renderTenants=renderTenants;window.getDueDateObj=getDueDateObj;window.getDueDate=getDueDate;window.getDueStatus=getDueStatus;window.getRentTab=getRentTab;window.getPeriodDates=getPeriodDates;window.getFullPaymentPool=getFullPaymentPool;window.renderRentRow=renderRentRow;window.calcLateFee=calcLateFee;window.togglePaymentMethod=togglePaymentMethod;window.openLateFeeSettings=openLateFeeSettings;window.saveLateFeeSettings=saveLateFeeSettings;window.bulkChaseOverdue=bulkChaseOverdue;window.shareCashCollections=shareCashCollections;window.setRentPeriod=setRentPeriod;window.setRentTab=setRentTab;window.setRentTabBtn=setRentTabBtn;window.setRentPeriodBtn=setRentPeriodBtn;window.setImportTab=setImportTab;window.toggleUserStatusBtn=toggleUserStatusBtn;window.switchUserBtn=switchUserBtn;window.renderRent=renderRent;window.renderExpenses=renderExpenses;window.renderMaintenance=renderMaintenance;window.getNextPayDate=getNextPayDate;window.dateToStr=dateToStr;window.generateSchedule=generateSchedule;window.rebuildAllSchedules=rebuildAllSchedules;window.rebuildTenantSchedule=rebuildTenantSchedule;window.markSchedulePaid=markSchedulePaid;window.markPartialPaid=markPartialPaid;window.confirmPartialPaid=confirmPartialPaid;window.markPaid=markPaid;window.recalcProperty=recalcProperty;window.freeRoom=freeRoom;window.occupyRoom=occupyRoom;window.deleteMaintenanceJob=deleteMaintenanceJob;window.updMaint=updMaint;window.confirmExp=confirmExp;window.removeExp=removeExp;window.editExpModal=editExpModal;window.saveExpEdit=saveExpEdit;window.renderTenantDueDaySection=renderTenantDueDaySection;window.updateTenantDueDaySection=updateTenantDueDaySection;window.pdSetOwnership=pdSetOwnership;window.pdSetLetting=pdSetLetting;window.pdSetStrEnabled=pdSetStrEnabled;window.setPropOwnership=setPropOwnership;window.setPropLetting=setPropLetting;window.toggleNewLandlordFields=toggleNewLandlordFields;window.refreshMaintRoomDropdown=refreshMaintRoomDropdown;window.refreshMaintTenantInfo=refreshMaintTenantInfo;window.previewMaintModalPhoto=previewMaintModalPhoto;window.openModal=openModal;window.closeModal=closeModal;window.debouncedTenantSearch=debouncedTenantSearch;window.debouncedPropSearch=debouncedPropSearch;window.onFreqChange=onFreqChange;window.refreshRoomDropdown=refreshRoomDropdown;window.saveModal=saveModal;window.saveTenantDetail=saveTenantDetail;window.doMoveRoom=doMoveRoom;window.generatePortalPassword=generatePortalPassword;window.giveTenantNotice=giveTenantNotice;window.cancelTenantNotice=cancelTenantNotice;window.moveTenantToRoom=moveTenantToRoom;window.moveTenantOut=moveTenantOut;window.openEditMaintModal=openEditMaintModal;window.saveEditMaint=saveEditMaint;window.previewMaintInvoice=previewMaintInvoice;window.addMaintExtraPhotos=addMaintExtraPhotos;window.removeMaintExtraPhoto=removeMaintExtraPhoto;window.clearPendingInvoicePreview=clearPendingInvoicePreview;window.clickMaintInvInput=clickMaintInvInput;window.clickMaintPhotosInput=clickMaintPhotosInput;window.clearMaintInvoice=clearMaintInvoice;window.openEditPaymentModal=openEditPaymentModal;window.saveEditPayment=saveEditPayment;window.revertPayment=revertPayment;window.renderPropertiesDealView=renderPropertiesDealView;window._dealSaveInputsToScratch=_dealSaveInputsToScratch;window.recalcDealPage=recalcDealPage;window.propViewList=propViewList;window.propViewDeal=propViewDeal;window.switchRentPeriod=switchRentPeriod;window.switchLettingType=switchLettingType;window.switchDealType=switchDealType;window.resetDealPage=resetDealPage;window.saveDealScenario=saveDealScenario;window.loadDealScenario=loadDealScenario;window.deleteDealScenario=deleteDealScenario;window.renderPropFinanceTab=renderPropFinanceTab;window.archiveProperty=archiveProperty;window.deletePropPermanent=deletePropPermanent;window.restoreProperty=restoreProperty;window.archiveTenant=archiveTenant;window.deleteTenantPermanent=deleteTenantPermanent;window.restoreTenant=restoreTenant;window.savePropDetail=savePropDetail;window.updateRoomPrice=updateRoomPrice;window.updateRoomType=updateRoomType;window.toggleRoomStatus=toggleRoomStatus;window.addRoomToProp=addRoomToProp;window.markLandlordPaid=markLandlordPaid;window.ensureLandlordSchedule=ensureLandlordSchedule;window.renderLandlords=renderLandlords;window.openAddLandlordModal=openAddLandlordModal;window.saveNewLandlord=saveNewLandlord;window.deleteLandlord=deleteLandlord;window.openLandlordDetail=openLandlordDetail;window.saveLandlordDetail=saveLandlordDetail;window.getPostcode=getPostcode;window.registerMedia=registerMedia;window.getMediaByElem=getMediaByElem;window.getMedia=getMedia;window.buildGalleryUrl=buildGalleryUrl;window.stripHouseNo=stripHouseNo;window.shareRoomWA=shareRoomWA;window.shareAllRoomsWA=shareAllRoomsWA;window.renderRooms=renderRooms;window.renderImport=renderImport;window.renderImportProperties=renderImportProperties;window.renderImportTenants=renderImportTenants;window.cleanPhone=cleanPhone;window.cleanPostcode=cleanPostcode;window.cleanPropertyName=cleanPropertyName;window.hasPropCode=hasPropCode;window.handlePropertiesFile=handlePropertiesFile;window.deriveArea=deriveArea;window.handleTenantsFile=handleTenantsFile;window.parseDate=parseDate;window.parseCSVLine=parseCSVLine;window.confirmImportProperties=confirmImportProperties;window.confirmImportTenants=confirmImportTenants;window.renderUsers=renderUsers;window.updateUserRole=updateUserRole;window.toggleRolePage=toggleRolePage;window.toggleRolePerm=toggleRolePerm;window.toggleUserStatus=toggleUserStatus;window.openAddUserModal=openAddUserModal;window.saveNewUser=saveNewUser;window.getVoidDays=getVoidDays;window.renderComplianceWidget=renderComplianceWidget;window.renderDepositSummary=renderDepositSummary;window.renderVoidTracker=renderVoidTracker;window.generateAgreement=generateAgreement;window.closeAgreement=closeAgreement;window.generateExcludedLicence=generateExcludedLicence;window.clearSignature=clearSignature;window.saveRoomNotes=saveRoomNotes;window.removeRoomVideo=removeRoomVideo;window.openDataModal=openDataModal;window._renderDataModal=_renderDataModal;window._dmBrowse=_dmBrowse;window._dmDrop=_dmDrop;window._dmPickFile=_dmPickFile;window._dmProcessFile=_dmProcessFile;window._dmParseRows=_dmParseRows;window._dmExport=_dmExport;window._dmTemplate=_dmTemplate;window.exportData=exportData;window.importData=importData;window.buildPortfolioSnapshot=buildPortfolioSnapshot;window.renderAgentKeyPrompt=renderAgentKeyPrompt;window.renderAgentLoading=renderAgentLoading;window.renderAgentError=renderAgentError;window.renderAgentResult=renderAgentResult;window.analyzeLocalDeal=analyzeLocalDeal;window.renderLocalDealOutput=renderLocalDealOutput;window.runLocalDealAnalysis=runLocalDealAnalysis;window.runLocalPortfolioAnalysis=runLocalPortfolioAnalysis;window.renderDashboardAgent=renderDashboardAgent;window.gsSearch=gsSearch;window.gsKey=gsKey;window.gsGo=gsGo;window.renderReports=renderReports;window.renderReportPL=renderReportPL;window.renderReportArrears=renderReportArrears;window.renderReportCashFlow=renderReportCashFlow;window.renderReportForecast=renderReportForecast;window.exportReportCSV=exportReportCSV;window.saveState=saveState;window.showToast=showToast;window.getEmailConfig=getEmailConfig;window.saveEmailConfig=saveEmailConfig;window.renderEmailSettings=renderEmailSettings;window.saveEmailField=saveEmailField;window.toggleEmailTrigger=toggleEmailTrigger;window.previewEmailForTenant=previewEmailForTenant;window.renderSettings=renderSettings;window.uploadLogo=uploadLogo;window.removeLogo=removeLogo;window.saveBranding=saveBranding;window.openAddCompanyModal=openAddCompanyModal;window.openEditCompanyModal=openEditCompanyModal;window.saveNewCompany=saveNewCompany;window.saveEditCompany=saveEditCompany;window.deleteCompany=deleteCompany;window.openInviteUserModal=openInviteUserModal;window.editUserModal=editUserModal;window.saveEditUser=saveEditUser;window.updateUserRoleByEl=updateUserRoleByEl;window.previewDoc=previewDoc;window.openContractorProfile=openContractorProfile;window.openAddContractorModal=openAddContractorModal;window.openEditContractorModal=openEditContractorModal;window.saveContractor=saveContractor;window.deleteContractor=deleteContractor;window.sendToContractorModal=sendToContractorModal;window.markContractorUsed=markContractorUsed;window.shareMaintWA=shareMaintWA;window.shareAllMaintWA=shareAllMaintWA;window.shareAllPropDocs=shareAllPropDocs;window.clearSavedState=clearSavedState;window.runAfterSupabaseLoad=runAfterSupabaseLoad;window.getPropDocMeta=getPropDocMeta;window.getDaysUntilExpiry=getDaysUntilExpiry;window.uploadPropDocFromInput=uploadPropDocFromInput;window.removePropDoc=removePropDoc;window.fetchAiMessages=fetchAiMessages;window.syncUsersFromOrgMembers=syncUsersFromOrgMembers;window.resolveOrg=resolveOrg;window.supaDelete=supaDelete;window.openTenantDetail=openTenantDetail;window.openPropDetail=openPropDetail;window.runDealAI=runDealAI;window.syncRoomPhotosBackground=syncRoomPhotosBackground;window.handlePhotoUpload=handlePhotoUpload;window.removeRoomPhoto=removeRoomPhoto;window.handleVideoUpload=handleVideoUpload;window.uploadTenantDoc=uploadTenantDoc;window._dmConfirmImport=_dmConfirmImport;window.runAIAgent=runAIAgent;window._supaUpsert=_supaUpsert;window._doSupaSave=_doSupaSave;window.loadState=loadState;window.backfillPaymentDueDates=backfillPaymentDueDates;window.doLogOut=doLogOut;window.sendEmail=sendEmail;window.sendTestEmail=sendTestEmail;window.runRentReminderEmails=runRentReminderEmails;window.sendScheduledReport=sendScheduledReport;window.sendInvite=sendInvite;window.deleteUserBtn=deleteUserBtn;window.renderPropDocsTab=renderPropDocsTab;window.uploadPropDoc=uploadPropDoc; }
