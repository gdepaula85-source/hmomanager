...
// loadState now fetches from Supabase (async) — scoped to single tenant or org
async function loadTenantsFromSupabase(tenantId) {
  try {
    var query = SUPA_URL + '/rest/v1/tenants?status=neq.inactive&select=*';
    if (tenantId) query += '&id=eq.' + encodeURIComponent(tenantId);
...
async function doLogin(){
  ...
  try {
    ...
    // C4/C5/C6 FIX: Use server-side password verification instead of client-side plaintext comparison
    var loginResp = await fetch('/api/tenant-portal/login', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ tenantId: tenant.id, password: pass })
    });
    if (!loginResp.ok) {
      var loginErr = await loginResp.json().catch(function(){ return {}; });
      showError(errEl, loginErr.error || 'Incorrect password. Please try again.');
...
async function doSetPassword(){
  ...
  if(p1.length < 8){ showError(errEl,'Password must be at least 8 characters.'); return; }
  ...
  try {
    ...
    // C4/C5/C6 FIX: Use server-side password hashing instead of plaintext
    var setResp = await fetch('/api/tenant-portal/set-password', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ tenantId: tenant.id, password: p1 })
    });
    if (!setResp.ok) {
      var setErr = await setResp.json().catch(function(){ return {}; });
      showError(errEl, setErr.error || 'Failed to set password');
...
async function checkSession(){
  ...
  // Fetch only the logged-in tenant from Supabase
  var tenants = [];
  try {
    tenants = await loadTenantsFromSupabase(data.tenantId);
  } catch(e) {}
...
// MAINTENANCE
// ═══════════════════════════════════════════════════════
/** When tenant.property_id is null, resolve UUID from properties (name + org) for FK on maintenance. */
async function resolvePropertyIdForTenant(t){
  if (!t || t.propertyId) return t && t.propertyId ? t.propertyId : null;
  var name = t.property && String(t.property).trim();
  if (!t.orgId || !name) return null;
  try {
    var q = 'name=eq.'+encodeURIComponent(name)+'&org_id=eq.'+encodeURIComponent(t.orgId)+'&select=id&limit=1';
    var res = await fetch(SUPA_URL+'/rest/v1/properties?'+q, {
      headers: {'apikey': SUPA_KEY, 'Authorization': 'Bearer '+SUPA_KEY}
    });
    var rows = await res.json();
  ...
  } catch(e) { console.warn('resolvePropertyIdForTenant:', e); }
  return null;
}

function setMaintNavBadge(openCount){
  var maintBadge = document.getElementById('nav-maint-badge');
  if(!maintBadge) return;
  if(openCount > 0){
    maintBadge.style.display='block';
    maintBadge.textContent = String(openCount);
  } else {
...
async function renderMaintenance(){
  ...
  // Source of truth: Supabase maintenance table only (no localStorage merge — avoids phantom rows)
  var supabaseMaint = [];
  try {
    var maintQ = t.orgId
      ? 'and=(tenant_name.eq.'+encodeURIComponent(t.name)+',org_id.eq.'+encodeURIComponent(t.orgId)+')'
      : 'tenant_name=eq.'+encodeURIComponent(t.name);
    var res = await fetch(SUPA_URL+'/rest/v1/maintenance?'+maintQ+'&select=*&order=logged_date.desc&limit=50', {
      headers: {'apikey': SUPA_KEY, 'Authorization': 'Bearer '+SUPA_KEY}
    });
    var rows = await res.json();
    if(Array.isArray(rows)) {
      supabaseMaint = rows.map(function(r){
        return {id:r.id, tenantId:t.id, tenantName:r.tenant_name,
          ...
          reportedDate: r.logged_date ? new Date(r.logged_date).getTime() : Date.now()};
      });
    }
  } catch(e) {
    console.warn('renderMaintenance fetch failed:', e);
  }

  supabaseMaint.sort(function(a,b){ return (b.reportedDate||0)-(a.reportedDate||0); });
  var combined = supabaseMaint;

  var openMaint = combined.filter(function(m){ return (m.status||'open')==='open'; });
  setMaintNavBadge(openMaint.length);
...
async function submitMaintenance(){
  ...
  var roomNum = t.room == null || t.room === '' ? null : parseInt(String(t.room).replace(/[^\d]/g,''), 10);
  if (roomNum !== roomNum) roomNum = null;

  var notesFull = notes ? ('(Reported by tenant) ' + notes) : '(Reported by tenant)';

  var propertyId = t.propertyId || null;
  if (!propertyId && t.orgId && t.property) {
    propertyId = await resolvePropertyIdForTenant(t);
    if (propertyId) t.propertyId = propertyId;
  }

  // Only columns that exist on public.maintenance (see db/maintenance.sql) — extra keys cause 400 from PostgREST
  var row = {
    id: newId,
    org_id: t.orgId || null,
  ...
  var saved = false;
  if (!t.orgId) {
    console.warn('submitMaintenance: tenant missing org_id');
  } else {
    try {
  ...
  if (saved) {
    ...
    toast('⚠️ Could not submit (account not linked to an organisation). Contact your landlord.');
  } else {
    toast('⚠️ Could not submit. Please check your connection and try again.');
  }
  goTab('maintenance');
  renderMaintenance();
}
...
// DOCUMENTS
// ═══════════════════════════════════════════════════════
/** Match dashboard `02-tenant-vault-storage-names.js` — storage path: {SLUG}__tdoc_....ext */
function tenantPortalDocTypeToSlug(label){
  var map = {
    'Right to Rent': 'RTR',
    'Passport / ID': 'PID',
...
var TENANT_PORTAL_SLUG_LABELS = {
  RTR: 'Right to Rent', PID: 'Passport / ID', POA: 'Proof of Address', EREF: 'Employment Reference',
  LREF: 'Landlord Reference', TAPP: 'Tenancy Application', BANK: 'Bank Statement', NI: 'NI Number', OTH: 'Other'
};
function tenantPortalParseVaultFileName(fileName){
  if (!fileName) return { label: '', fromSlug: false };
  var m = fileName.match(/^([A-Z0-9]+)__(.+)$/i);
  if (!m) return { label: '', fromSlug: false };
  var slug = m[1].toUpperCase();
  var lab = TENANT_PORTAL_SLUG_LABELS[slug];
  if (lab) return { label: lab, fromSlug: true };
  return { label: '', fromSlug: false };
}
...
async function renderDocuments(){
  ...
  try {
    ...
    if(Array.isArray(files)){
      supabaseDocs = files.filter(function(f){return f.name&&!f.name.startsWith('.');}).map(function(f){
        var parsed = tenantPortalParseVaultFileName(f.name);
        var dispType = parsed.fromSlug ? parsed.label : 'Document';
        return {id:f.name,name:f.name,displayTitle:parsed.fromSlug?parsed.label:f.name,type:dispType,size:'',uploadedAt:'',
  ...
  if(!allDocs.length){
    ...
    docsHtml = allDocs.map(function(doc){
      var parsed = tenantPortalParseVaultFileName(doc.name);
      var headLine = doc.displayTitle || (parsed.fromSlug ? parsed.label : null) || doc.name;
      var metaType = doc.type && doc.type !== 'Document' ? doc.type : (parsed.fromSlug ? parsed.label : 'Document');
      var showDup = metaType && metaType !== 'Document' && headLine !== metaType;
      var icon = (doc.name||'').match(/\.pdf$/i)?'\uD83D\uDCC4':(doc.name||'').match(/\.(jpg|jpeg|png)$/i)?'\uD83D\uDDBC\uFE0F':'\uD83D\uDCCB';
      var bg   = typeColors[metaType]||'#F1F5F9';
      var col  = typeTextColors[metaType]||'#475569';
      var metaBits = [];
      if(showDup) metaBits.push('<span style="color:'+col+';font-weight:700">'+metaType+'</span>');
      if(doc.size) metaBits.push(doc.size);
      if(doc.uploadedAt) metaBits.push(doc.uploadedAt);
      return '<div class="doc-row">'
        +'<div class="doc-icon" style="background:'+bg+'">'+icon+'</div>'
        +'<div class="doc-info"><div class="doc-name">'+headLine+'</div>'
        +'<div class="doc-meta">'+metaBits.join(' &middot; ')+'</div></div>'
...
async function handleDocUpload(input){
  ...
  for(var fi=0; fi<files.length; fi++){
    var file = files[fi];
    if(file.size>10*1024*1024){ toast('⚠️ '+file.name+' is too large (max 10MB)'); continue; }
    var docId = 'tdoc_'+Date.now()+'_'+fi+'_'+Math.random().toString(36).slice(2,6);
    var ext = (file.name.split('.').pop()||'bin').toLowerCase();
    var slug = tenantPortalDocTypeToSlug(docType);
    var path = 'tenants/'+String(t.id)+'/'+slug+'__'+docId+'.'+ext;
    var sizeStr = file.size>1024*1024?(file.size/1024/1024).toFixed(1)+'MB':Math.round(file.size/1024)+'KB';
    var uploadedAt = new Date().toLocaleDateString('en-GB',{day:'2-digit',month:'short',year:'numeric'});
    try {
      var upRes = await supa.storage.from('tenant-docs').upload(path, file, { upsert:true, cacheControl:'3600' });
      if(upRes.error) throw upRes.error;
      var pub = supa.storage.from('tenant-docs').getPublicUrl(path);
      var dataUrl = pub.data && pub.data.publicUrl ? pub.data.publicUrl : null;
      if(!dataUrl) {
        var sign = await supa.storage.from('tenant-docs').createSignedUrl(path, 31536000);
        if(sign.data && sign.data.signedUrl) dataUrl = sign.data.signedUrl;
      }
      if(!dataUrl) throw new Error('Could not get file URL');
      vault[t.id].push({
        id:docId, name:file.name, displayTitle:docType, type:docType, size:sizeStr, uploadedAt:uploadedAt,
        dataUrl:dataUrl, storagePath:path
      });
      uploaded++;
    } catch(err){
      console.warn('Tenant doc upload:', err);
      toast('⚠️ Upload failed for '+file.name+'. '+(err.message||''));
    }
  }
  saveVault(vault);
  await renderDocuments();
  if(uploaded) toast('✅ '+uploaded+' document'+(uploaded===1?'':'s')+' uploaded!');
}
...
async function deleteTenantDoc(docId){
  ...
  var vault = loadVault();
  var list = vault[t.id]||[];
  var doc = list.find(function(d){ return d.id===docId; });
  if(doc && doc.storagePath) {
    try {
      await supa.storage.from('tenant-docs').remove([doc.storagePath]);
    } catch(e){ console.warn('Storage remove:', e); }
  }
  vault[t.id] = list.filter(function(d){ return d.id!==docId; });
  saveVault(vault);
  await renderDocuments();
...
async function saveProfile(){
  var t = currentTenant;
  if(!t || !t.id){ toast('⚠️ Not signed in'); return; }
  var name = document.getElementById('p-name').value.trim()||t.name;
  var email = document.getElementById('p-email').value.trim();
  var whatsapp = document.getElementById('p-wa').value.trim().replace(/\s+/g,'').replace(/^\+/,'');

  try {
    var res = await fetch(SUPA_URL+'/rest/v1/tenants?id=eq.'+encodeURIComponent(t.id), {
      method: 'PATCH',
      headers: {
        'apikey': SUPA_KEY,
    ...
    if(!res.ok) {
      ...
      console.warn('saveProfile:', res.status, errTxt);
      toast('⚠️ Could not save profile. Try again or contact your property manager.');
      return;
    }
  } catch(e) {
    console.warn('saveProfile error:', e);
    toast('⚠️ Could not save profile. Check your connection.');
    return;
  }

  currentTenant.name = name;
  currentTenant.email = email;
  currentTenant.whatsapp = whatsapp;

  try {
    var cachedRaw = localStorage.getItem('pm_portal_tenant_cache');
    if(cachedRaw) {
      var arr = JSON.parse(cachedRaw);
      var ix = arr.findIndex(function(x){ return String(x.id)===String(t.id); });
      if(ix>=0) {
        arr[ix].name = name; arr[ix].email = email; arr[ix].whatsapp = whatsapp;
        localStorage.setItem('pm_portal_tenant_cache', JSON.stringify(arr));
      }
    }
  } catch(e){}

  var tenants = loadState();
  var idx = tenants.findIndex(function(x){ return String(x.id)===String(t.id); });
  if(idx>=0) {
    tenants[idx].name = name;
    tenants[idx].email = email;
    tenants[idx].whatsapp = whatsapp;
    saveTenants(tenants);
  }
...
async function changePassword(){
  ...
  if(newPass.length<8){ toast('⚠️ New password must be at least 8 characters'); return; }
  if(newPass!==confPass){ toast('⚠️ New passwords do not match'); return; }
  var t = currentTenant;
  try {
    var res = await fetch('/api/tenant-portal/set-password', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ tenantId: t.id, password: newPass, currentPassword: oldPass })
    });
    if(!res.ok) {
      var err = await res.json().catch(function(){ return {}; });
      toast('⚠️ ' + (err.error || 'Could not update password. Try again.'));
      return;
    }
  } catch(e) {
    toast('⚠️ Could not update password. Check your connection.');
    return;
  }
  t.portalPassword = null;
...
async function cancelNoticeRequest(){
  var t = currentTenant;
  var tenants = loadState();
  ...
  saveTenants(tenants);
  // H11 FIX: Also update Supabase so the manager sees the cancellation
  try {
    await fetch(SUPA_URL + '/rest/v1/tenants?id=eq.' + t.id, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json', 'apikey': SUPA_KEY, 'Authorization': 'Bearer ' + SUPA_KEY },
      body: JSON.stringify({ status: 'active', notice_date: null, move_out_date: null })
    });
  } catch(e) { console.warn('cancelNotice supabase sync failed:', e); }
  renderAll();
...
function updateBadges(){
  ...
  // Maintenance badge is updated when renderMaintenance() loads from Supabase (setMaintNavBadge)
}
